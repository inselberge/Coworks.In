    ...
    implements concreteModelInterface
{
    ...
    static protected $acceptableKeys = array("id", "user_id", "title", "description", "url", "status", "create_date", "modify_date", "modify_user", "modify_reason");
    ...
    */
    ...
    public function get( $id ) {
        try {
            if ( checkInput::checkNewInput($id) ) {
    	           if ( $this->query->execute("CALL getCompany('$id')") ) {
    	               $this->query->getRow();
    ...
    public function update()
    {
        try {
            if ( $this->checkRequired() ) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL updateCompany(
                        '$this->id'
                        ,'$this->user_id'
                        ,'$this->title'
                        ,'$this->description'
    ...
    public function insert()
    {
        try {
            if ( $this->checkRequired()) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL insertCompany(
                        '$this->user_id'
                        ,'$this->title'
                        ,'$this->description'
                        ,'$this->url'
    ...
    public function validate( $id )
    {
        try {
            if ( checkInput::checkNewInput($id) ) {
                if ( $this->query->execute("CALL validateCompany('$id')") ) {
                    $this->query->checkAffected();
                } else {
    ...
    {
        try {
            if (checkInput::checkNewInputArray(array('$this->user_id','$this->title','$this->description','$this->url','$this->status','$this->create_date'))) {
            } else {
