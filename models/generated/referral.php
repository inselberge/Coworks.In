    ...
    */
    ...
    public function get( $id ) {
        try {
            if ( checkInput::checkNewInput($id) ) {
    	           if ( $this->query->execute("CALL getReferral('$id')") ) {
    	               $this->query->getRow();
    ...
    public function update()
    {
        try {
            if ( $this->checkRequired() ) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL updateReferral(
                        '$this->id'
                        ,'$this->user_id'
                        ,'$this->event_id'
                        ,'$this->key'
    ...
    public function insert()
    {
        try {
            if ( $this->checkRequired()) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL insertReferral(
                        '$this->user_id'
                        ,'$this->event_id'
                        ,'$this->key'
                        ,'$this->clicks'
    ...
    public function validate( $id )
    {
        try {
            if ( checkInput::checkNewInput($id) ) {
                if ( $this->query->execute("CALL validateReferral('$id')") ) {
                    $this->query->checkAffected();
                } else {
    ...
    {
        try {
            if (checkInput::checkNewInputArray(array('$this->user_id','$this->event_id','$this->key','$this->clicks','$this->status','$this->create_date'))) {
            } else {
