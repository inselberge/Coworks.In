...
/**
 * Class Event_VisibilityModel
 *
 * @category Model
 * @package  Event_Visibility
 * @author   
 * @license  TBD
 * @link     TBD
 */
    ...
    implements concreteModelInterface
{
    ...
    static public $Event_VisibilityModel;
    static protected $acceptableKeys = array("id", "event_id", "thread", "visibility", "create_date", "modify_date", "modify_user", "modify_reason");
    ...
    */
    static public function instance($dbc) {
        if(!isset(self::$Event_VisibilityModel) ) {
            self::$Event_VisibilityModel = new Event_VisibilityModel($dbc);
        } else {
            self::$Event_VisibilityModel->connection = $dbc;
        }
        return self::$Event_VisibilityModel;
    }
        ...
        if(isset($key,$value) AND !is_null($key)){
            switch($key){
                ...
                case 'thread' :
                    ...
                    break;
                case 'visibility' :
                    if($this->checkValue("enum('public','protected','private','exclusive')",4,$value)){
            ...
            if ( checkInput::checkNewInputArray(array($type, $max, $value)) ) {
                switch ( $type ) {
                    case "int(10) unsigned":
                        ...
                        break;
                    case "enum('public','protected','private','exclusive')":
                        if (  in_array($value,array( 'public' , 'protected' , 'private' , 'exclusive' ))  ) {
                            return true;
                        } else {
                            return false;
                        }
    ...
    public function get( $id ) {
        try {
            if ( checkInput::checkNewInput($id) ) {
    	           if ( $this->query->execute("CALL getEvent_Visibility('$id')") ) {
    	               $this->query->getRow();
    ...
    public function update()
    {
        try {
            if ( $this->checkRequired() ) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL updateEvent_Visibility(
                        '$this->id'
                        ,'$this->event_id'
                        ,'$this->thread'
                        ,'$this->visibility'
    ...
    public function insert()
    {
        try {
            if ( $this->checkRequired()) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL insertEvent_Visibility(
                        '$this->event_id'
                        ,'$this->visibility'
                            , @ceid)")
                    ) {
    ...
    public function validate( $id )
    {
        try {
            if ( checkInput::checkNewInput($id) ) {
                if ( $this->query->execute("CALL validateEvent_Visibility('$id')") ) {
                    $this->query->checkAffected();
                } else {
    ...
    {
        try {
            if (checkInput::checkNewInputArray(array('$this->event_id','$this->visibility','$this->create_date'))) {
            } else {
