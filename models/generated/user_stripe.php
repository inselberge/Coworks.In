    ...
    */
    ...
    public function get( $id ) {
        try {
            if ( checkInput::checkNewInput($id) ) {
    	           if ( $this->query->execute("CALL getUser_Stripe('$id')") ) {
    	               $this->query->getRow();
    ...
    public function update()
    {
        try {
            if ( $this->checkRequired() ) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL updateUser_Stripe(
                        '$this->id'
                        ,'$this->user_id'
                        ,'$this->stripe_token'
                        ,'$this->last_checked'
    ...
    public function insert()
    {
        try {
            if ( $this->checkRequired()) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL insertUser_Stripe(
                        '$this->user_id'
                        ,'$this->stripe_token'
                        ,'$this->last_checked'
                        ,'$this->is_active'
    ...
    public function validate( $id )
    {
        try {
            if ( checkInput::checkNewInput($id) ) {
                if ( $this->query->execute("CALL validateUser_Stripe('$id')") ) {
                    $this->query->checkAffected();
                } else {
    ...
    {
        try {
            if (checkInput::checkNewInputArray(array('$this->user_id','$this->stripe_token','$this->last_checked','$this->is_active','$this->create_date'))) {
            } else {
