    ...
    */
    ...
    public function get( $id ) {
        try {
            if ( checkInput::checkNewInput($id) ) {
    	           if ( $this->query->execute("CALL getMentor_Availability('$id')") ) {
    	               $this->query->getRow();
    ...
    public function update()
    {
        try {
            if ( $this->checkRequired() ) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL updateMentor_Availability(
                        '$this->id'
                        ,'$this->user_id'
                        ,'$this->start_time'
                        ,'$this->end_time'
    ...
    public function insert()
    {
        try {
            if ( $this->checkRequired()) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL insertMentor_Availability(
                        '$this->user_id'
                        ,'$this->start_time'
                        ,'$this->end_time'
                        ,'$this->status'
    ...
    public function validate( $id )
    {
        try {
            if ( checkInput::checkNewInput($id) ) {
                if ( $this->query->execute("CALL validateMentor_Availability('$id')") ) {
                    $this->query->checkAffected();
                } else {
    ...
    {
        try {
            if (checkInput::checkNewInputArray(array('$this->user_id','$this->start_time','$this->end_time','$this->status','$this->create_date'))) {
            } else {
