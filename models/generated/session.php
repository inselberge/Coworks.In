    ...
    implements concreteModelInterface
{
    ...
    static public $SessionModel;
    static protected $acceptableKeys = array("id", "coworking_space_id", "session_id", "ip", "page_views", "is_member", "user_id", "create_date", "modify_date");
    ...
    */
    static public function instance($dbc) {
        if(!isset(self::$SessionModel) ) {
            self::$SessionModel = new SessionModel($dbc);
        } else {
            self::$SessionModel->connection = $dbc;
        }
    ...
    public function get( $id ) {
        try {
            if ( checkInput::checkNewInput($id) ) {
    	           if ( $this->query->execute("CALL getSession('$id')") ) {
    	               $this->query->getRow();
    ...
    public function update()
    {
        try {
            if ( $this->checkRequired() ) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL updateSession(
                        '$this->id'
    ...
    public function insert()
    {
        try {
            if ( $this->checkRequired()) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL insertSession(
                        '$this->session_id'
                        ,'$this->ip'
                        ,'$this->page_views'
                        ,'$this->is_member'
    ...
    public function validate( $id )
    {
        try {
            if ( checkInput::checkNewInput($id) ) {
                if ( $this->query->execute("CALL validateSession('$id')") ) {
                    $this->query->checkAffected();
                } else {
    ...
    {
        try {
            if (checkInput::checkNewInputArray(array('$this->session_id','$this->ip','$this->page_views','$this->is_member','$this->create_date'))) {
            } else {
