    ...
    */
    ...
    public function setValue($key,$value) {
        if(isset($key,$value) AND !is_null($key)){
            switch($key){
                ...
                case 'coworking_id' :
                    if($this->checkValue("int(10) unsigned",10,$value)){
                        $this->coworking_id=$value;
                    } else {
                        return false;
                    };
                    break;
                case 'live' :
                    if($this->checkValue("enum('test','live')",2,$value)){
                        $this->live=$value;
                    } else {
                        return false;
                    };
                    break;
                case 'type' :
                    if($this->checkValue("enum('public','private')",2,$value)){
                        $this->type=$value;
                    } else {
                        return false;
                    };
            ...
            if ( checkInput::checkNewInputArray(array($type, $max, $value)) ) {
                switch ( $type ) {
                    case "int(10) unsigned":
                        ...
                        break;
                    case "enum('test','live')":
                        if (  in_array($value,array( 'test' , 'live' ))  ) {
                            return true;
                        } else {
                            return false;
                        }
                        break;
                    case "enum('public','private')":
    ...
    public function get( $id ) {
        try {
            if ( checkInput::checkNewInput($id) ) {
    	           if ( $this->query->execute("CALL getCoworking_Stripe('$id')") ) {
    	               $this->query->getRow();
    ...
    public function update()
    {
        try {
            if ( $this->checkRequired() ) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL updateCoworking_Stripe(
                        '$this->id'
                        ,'$this->coworking_id'
                        ,'$this->live'
                        ,'$this->type'
    ...
    public function insert()
    {
        try {
            if ( $this->checkRequired()) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL insertCoworking_Stripe(
                        '$this->coworking_id'
                        ,'$this->live'
                        ,'$this->type'
                        ,'$this->key'
    ...
    public function validate( $id )
    {
        try {
            if ( checkInput::checkNewInput($id) ) {
                if ( $this->query->execute("CALL validateCoworking_Stripe('$id')") ) {
                    $this->query->checkAffected();
                } else {
    ...
    {
        try {
            if (checkInput::checkNewInputArray(array('$this->coworking_id','$this->live','$this->type','$this->key','$this->status','$this->create_date'))) {
            } else {
