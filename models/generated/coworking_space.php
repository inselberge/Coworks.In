    ...
    implements concreteModelInterface
{
    ...
    static protected $acceptableKeys = array("id", "user_id", "name", "description", "url", "user_url_stub", "status", "create_date", "modify_date", "modify_user", "modify_reason");
    ...
    */
    ...
    public function get( $id ) {
        try {
            if ( checkInput::checkNewInput($id) ) {
    	           if ( $this->query->execute("CALL getCoworking_Space('$id')") ) {
    	               $this->query->getRow();
    ...
    public function update()
    {
        try {
            if ( $this->checkRequired() ) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL updateCoworking_Space(
                        '$this->id'
                        ,'$this->user_id'
                        ,'$this->name'
                        ,'$this->description'
    ...
    public function insert()
    {
        try {
            if ( $this->checkRequired()) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL insertCoworking_Space(
                        '$this->user_id'
                        ,'$this->name'
                        ,'$this->description'
                        ,'$this->url'
    ...
    public function validate( $id )
    {
        try {
            if ( checkInput::checkNewInput($id) ) {
                if ( $this->query->execute("CALL validateCoworking_Space('$id')") ) {
                    $this->query->checkAffected();
                } else {
    ...
    {
        try {
            if (checkInput::checkNewInputArray(array('$this->user_id','$this->name','$this->description','$this->url','$this->user_url_stub','$this->status','$this->create_date'))) {
            } else {
