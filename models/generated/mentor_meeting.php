    ...
    */
        ...
        if(isset($key,$value) AND !is_null($key)){
            switch($key){
                case 'id' :
                    ...
                    break;
                case 'mentor_availability_id' :
                    if($this->checkValue("int(10) unsigned",10,$value)){
                        $this->mentor_availability_id=$value;
                    } else {
                        return false;
                    };
                    break;
    ...
    public function get( $id ) {
        try {
            if ( checkInput::checkNewInput($id) ) {
    	           if ( $this->query->execute("CALL getMentor_Meeting('$id')") ) {
    	               $this->query->getRow();
    ...
    public function update()
    {
        try {
            if ( $this->checkRequired() ) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL updateMentor_Meeting(
                        '$this->id'
                        ,'$this->mentor_availability_id'
                        ,'$this->user_id'
                        ,'$this->status'
                        ,'$this->create_date'
                        ,'$this->modify_date'
    ...
    public function insert()
    {
        try {
            if ( $this->checkRequired()) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL insertMentor_Meeting(
                        '$this->mentor_availability_id'
                        ,'$this->user_id'
                        ,'$this->status'
                            , @ceid)")
    ...
    public function validate( $id )
    {
        try {
            if ( checkInput::checkNewInput($id) ) {
                if ( $this->query->execute("CALL validateMentor_Meeting('$id')") ) {
                    $this->query->checkAffected();
                } else {
    ...
    {
        try {
            if (checkInput::checkNewInputArray(array('$this->mentor_availability_id','$this->user_id','$this->status','$this->create_date'))) {
            } else {
