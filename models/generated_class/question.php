...
class QuestionModel extends basicModel implements concreteModelInterface {
    ...
    public function get( $id ) {
        try {
            if ( checkInput::checkNewInput($id) ) {
    	           if ( $this->query->execute("CALL getQuestion('$id')") ) {
    	               $this->query->getRow();
    ...
    public function update()
    {
        try {
            if ( $this->checkRequired() ) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL updateQuestion(
                        '$this->id'
                        ,'$this->user_id'
                        ,'$this->title'
                        ,'$this->description'
    ...
    */
    public function insert()
    {
        try {
            if ( $this->checkRequired()) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL insertQuestion(
                        '$this->user_id'
                        ,'$this->title'
                        ,'$this->description'
                        ,'$this->status'
    ...
    */
    public function validate( $id )
    {
        try {
            if ( checkInput::checkNewInput($id) ) {
                if ( $this->query->execute("CALL validateQuestion('$id')") ) {
                    $this->query->checkAffected();
                } else {
