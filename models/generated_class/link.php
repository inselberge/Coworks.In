...
Class: LinkModel
******************/
class LinkModel extends basicModel implements concreteModelInterface {
    /*** Attributes: ***/
    private $id;
    protected $user_id;
    protected $url;
    ...
    static public $LinkModel;
    static protected $acceptableKeys = array("id", "user_id", "url", "clicks", "status", "create_date", "modify_date", "modify_user", "modify_reason");
    ...
    */
    static public function instance($dbc) {
        if(!isset(self::$LinkModel) ) {
            self::$LinkModel = new LinkModel($dbc);
        } else {
            self::$LinkModel->connection = $dbc;
        }
    ...
    public function get( $id ) {
        try {
            if ( checkInput::checkNewInput($id) ) {
    	           if ( $this->query->execute("CALL getLink('$id')") ) {
    	               $this->query->getRow();
    ...
    public function update()
    {
        try {
            if ( $this->checkRequired() ) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL updateLink(
                        '$this->id'
                        ,'$this->user_id'
                        ,'$this->url'
                        ,'$this->clicks'
    ...
    */
    public function insert()
    {
        try {
            if ( $this->checkRequired()) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL insertLink(
                        '$this->user_id'
                        ,'$this->url'
                        ,'$this->clicks'
                        ,'$this->status'
    ...
    */
    public function validate( $id )
    {
        try {
            if ( checkInput::checkNewInput($id) ) {
                if ( $this->query->execute("CALL validateLink('$id')") ) {
                    $this->query->checkAffected();
                } else {
    ...
    */
    ...
    {
        try {
            if (checkInput::checkNewInputArray(array('$this->user_id','$this->url','$this->clicks','$this->status','$this->create_date'))) {
            } else {
