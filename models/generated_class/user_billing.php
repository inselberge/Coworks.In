...
/******************
Class: User_BillingModel
******************/
class User_BillingModel extends basicModel implements concreteModelInterface {
    /*** Attributes: ***/
    private $id;
    protected $user_id;
    protected $coworking_space_billing_plan_id;
    protected $status;
    protected $create_date;
    protected $modify_date;
    protected $modify_user;
    protected $modify_reason;
    static public $User_BillingModel;
    static protected $acceptableKeys = array("id", "user_id", "coworking_space_billing_plan_id", "status", "create_date", "modify_date", "modify_user", "modify_reason");
    ...
    */
    static public function instance($dbc) {
        if(!isset(self::$User_BillingModel) ) {
            self::$User_BillingModel = new User_BillingModel($dbc);
        } else {
            self::$User_BillingModel->connection = $dbc;
        }
        return self::$User_BillingModel;
    }
        ...
        if(isset($key,$value) AND !is_null($key)){
            switch($key){
                ...
                case 'user_id' :
                    ...
                    break;
                case 'coworking_space_billing_plan_id' :
                    if($this->checkValue("int(10) unsigned",10,$value)){
                        $this->coworking_space_billing_plan_id=$value;
                    } else {
                        return false;
                    };
                    break;
    ...
    public function get( $id ) {
        try {
            if ( checkInput::checkNewInput($id) ) {
    	           if ( $this->query->execute("CALL getUser_Billing('$id')") ) {
    	               $this->query->getRow();
    ...
    public function update()
    {
        try {
            if ( $this->checkRequired() ) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL updateUser_Billing(
                        '$this->id'
                        ,'$this->user_id'
                        ,'$this->coworking_space_billing_plan_id'
                        ,'$this->status'
                        ,'$this->create_date'
                        ,'$this->modify_date'
                        ,'$this->modify_user'
    ...
    */
    public function insert()
    {
        try {
            if ( $this->checkRequired()) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL insertUser_Billing(
                        '$this->user_id'
                        ,'$this->coworking_space_billing_plan_id'
                        ,'$this->status'
                            , @ceid)")
    ...
    */
    public function validate( $id )
    {
        try {
            if ( checkInput::checkNewInput($id) ) {
                if ( $this->query->execute("CALL validateUser_Billing('$id')") ) {
                    $this->query->checkAffected();
                } else {
    ...
    */
    ...
    {
        try {
            if (checkInput::checkNewInputArray(array('$this->user_id','$this->coworking_space_billing_plan_id','$this->status','$this->create_date'))) {
            } else {
