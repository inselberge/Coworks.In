...
class Help_TagsModel extends basicModel implements concreteModelInterface {
    ...
    public function get( $id ) {
        try {
            if ( checkInput::checkNewInput($id) ) {
    	           if ( $this->query->execute("CALL getHelp_Tags('$id')") ) {
    	               $this->query->getRow();
    ...
    public function update()
    {
        try {
            if ( $this->checkRequired() ) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL updateHelp_Tags(
                        '$this->id'
                        ,'$this->help_id'
                        ,'$this->tag_id'
                        ,'$this->create_date'
    ...
    */
    public function insert()
    {
        try {
            if ( $this->checkRequired()) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL insertHelp_Tags(
                        '$this->help_id'
                        ,'$this->tag_id'
                            , @ceid)")
                    ) {
    ...
    */
    public function validate( $id )
    {
        try {
            if ( checkInput::checkNewInput($id) ) {
                if ( $this->query->execute("CALL validateHelp_Tags('$id')") ) {
                    $this->query->checkAffected();
                } else {
    ...
    */
    ...
    {
        try {
            if (checkInput::checkNewInputArray(array('$this->help_id','$this->tag_id','$this->create_date'))) {
            } else {
