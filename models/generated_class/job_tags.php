...
class Job_TagsModel extends basicModel implements concreteModelInterface {
    ...
    public function get( $id ) {
        try {
            if ( checkInput::checkNewInput($id) ) {
    	           if ( $this->query->execute("CALL getJob_Tags('$id')") ) {
    	               $this->query->getRow();
    ...
    public function update()
    {
        try {
            if ( $this->checkRequired() ) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL updateJob_Tags(
                        '$this->id'
                        ,'$this->job_id'
                        ,'$this->tag_id'
                        ,'$this->create_date'
    ...
    */
    public function insert()
    {
        try {
            if ( $this->checkRequired()) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL insertJob_Tags(
                        '$this->job_id'
                        ,'$this->tag_id'
                            , @ceid)")
                    ) {
    ...
    */
    public function validate( $id )
    {
        try {
            if ( checkInput::checkNewInput($id) ) {
                if ( $this->query->execute("CALL validateJob_Tags('$id')") ) {
                    $this->query->checkAffected();
                } else {
    ...
    */
    ...
    {
        try {
            if (checkInput::checkNewInputArray(array('$this->job_id','$this->tag_id','$this->create_date'))) {
            } else {
