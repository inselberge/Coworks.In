...
/******************
Class: Question_TagsModel
******************/
class Question_TagsModel extends basicModel implements concreteModelInterface {
    /*** Attributes: ***/
    private $id;
    ...
    static public $Question_TagsModel;
    static protected $acceptableKeys = array("id", "question_id", "tag_id", "status", "create_date", "modify_date", "modify_user", "modify_reason");
    ...
    */
    static public function instance($dbc) {
        if(!isset(self::$Question_TagsModel) ) {
            self::$Question_TagsModel = new Question_TagsModel($dbc);
        } else {
            self::$Question_TagsModel->connection = $dbc;
        }
        return self::$Question_TagsModel;
    }
    ...
    public function get( $id ) {
        try {
            if ( checkInput::checkNewInput($id) ) {
    	           if ( $this->query->execute("CALL getQuestion_Tags('$id')") ) {
    	               $this->query->getRow();
    ...
    public function update()
    {
        try {
            if ( $this->checkRequired() ) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL updateQuestion_Tags(
                        '$this->id'
                        ,'$this->question_id'
                        ,'$this->tag_id'
                        ,'$this->status'
    ...
    */
    public function insert()
    {
        try {
            if ( $this->checkRequired()) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL insertQuestion_Tags(
                        '$this->question_id'
                        ,'$this->tag_id'
                        ,'$this->status'
                            , @ceid)")
    ...
    */
    public function validate( $id )
    {
        try {
            if ( checkInput::checkNewInput($id) ) {
                if ( $this->query->execute("CALL validateQuestion_Tags('$id')") ) {
                    $this->query->checkAffected();
                } else {
    ...
    */
    ...
    {
        try {
            if (checkInput::checkNewInputArray(array('$this->question_id','$this->tag_id','$this->status','$this->create_date'))) {
            } else {
