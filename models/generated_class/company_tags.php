...
/******************
Class: Company_TagsModel
******************/
class Company_TagsModel extends basicModel implements concreteModelInterface {
    /*** Attributes: ***/
    private $id;
    ...
    static public $Company_TagsModel;
    static protected $acceptableKeys = array("id", "company_id", "tag_id", "status", "create_date", "modify_date", "modify_user", "modify_reason");
    ...
    */
    static public function instance($dbc) {
        if(!isset(self::$Company_TagsModel) ) {
            self::$Company_TagsModel = new Company_TagsModel($dbc);
        } else {
            self::$Company_TagsModel->connection = $dbc;
        }
        return self::$Company_TagsModel;
    }
    ...
    public function get( $id ) {
        try {
            if ( checkInput::checkNewInput($id) ) {
    	           if ( $this->query->execute("CALL getCompany_Tags('$id')") ) {
    	               $this->query->getRow();
    ...
    public function update()
    {
        try {
            if ( $this->checkRequired() ) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL updateCompany_Tags(
                        '$this->id'
                        ,'$this->company_id'
                        ,'$this->tag_id'
                        ,'$this->status'
    ...
    */
    public function insert()
    {
        try {
            if ( $this->checkRequired()) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL insertCompany_Tags(
                        '$this->company_id'
                        ,'$this->tag_id'
                        ,'$this->status'
                            , @ceid)")
    ...
    */
    public function validate( $id )
    {
        try {
            if ( checkInput::checkNewInput($id) ) {
                if ( $this->query->execute("CALL validateCompany_Tags('$id')") ) {
                    $this->query->checkAffected();
                } else {
    ...
    */
    ...
    {
        try {
            if (checkInput::checkNewInputArray(array('$this->company_id','$this->tag_id','$this->status','$this->create_date'))) {
            } else {
