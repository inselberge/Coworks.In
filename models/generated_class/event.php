...
Class: EventModel
******************/
class EventModel extends basicModel implements concreteModelInterface {
    /*** Attributes: ***/
    private $id;
    protected $user_id;
    protected $title;
    ...
    static public $EventModel;
    static protected $acceptableKeys = array("id", "user_id", "title", "description", "start_time", "end_time", "url", "status", "create_date", "thread", "modify_date", "modify_user", "modify_reason");
    ...
    */
    static public function instance($dbc) {
        if(!isset(self::$EventModel) ) {
            self::$EventModel = new EventModel($dbc);
        } else {
            self::$EventModel->connection = $dbc;
        }
    ...
    public function get( $id ) {
        try {
            if ( checkInput::checkNewInput($id) ) {
    	           if ( $this->query->execute("CALL getEvent('$id')") ) {
    	               $this->query->getRow();
    ...
    public function update()
    {
        try {
            if ( $this->checkRequired() ) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL updateEvent(
                        '$this->id'
                        ,'$this->user_id'
                        ,'$this->title'
                        ,'$this->description'
    ...
    */
    public function insert()
    {
        try {
            if ( $this->checkRequired()) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL insertEvent(
                        '$this->user_id'
                        ,'$this->title'
                        ,'$this->description'
                        ,'$this->start_time'
    ...
    */
    public function validate( $id )
    {
        try {
            if ( checkInput::checkNewInput($id) ) {
                if ( $this->query->execute("CALL validateEvent('$id')") ) {
                    $this->query->checkAffected();
                } else {
    ...
    */
    ...
    {
        try {
            if (checkInput::checkNewInputArray(array('$this->user_id','$this->title','$this->description','$this->start_time','$this->end_time','$this->url','$this->status','$this->create_date'))) {
            } else {
