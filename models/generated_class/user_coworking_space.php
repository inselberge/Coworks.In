...
/******************
Class: User_Coworking_SpaceModel
******************/
class User_Coworking_SpaceModel extends basicModel implements concreteModelInterface {
    /*** Attributes: ***/
    private $id;
    protected $user_id;
    ...
    static public $User_Coworking_SpaceModel;
    static protected $acceptableKeys = array("id", "user_id", "coworking_space_id", "status", "create_date", "modify_date", "modify_user", "modify_reason");
    ...
    */
    static public function instance($dbc) {
        if(!isset(self::$User_Coworking_SpaceModel) ) {
            self::$User_Coworking_SpaceModel = new User_Coworking_SpaceModel($dbc);
        } else {
            self::$User_Coworking_SpaceModel->connection = $dbc;
        }
        return self::$User_Coworking_SpaceModel;
    }
    ...
    public function get( $id ) {
        try {
            if ( checkInput::checkNewInput($id) ) {
    	           if ( $this->query->execute("CALL getUser_Coworking_Space('$id')") ) {
    	               $this->query->getRow();
    ...
    public function update()
    {
        try {
            if ( $this->checkRequired() ) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL updateUser_Coworking_Space(
                        '$this->id'
                        ,'$this->user_id'
    ...
    */
    public function insert()
    {
        try {
            if ( $this->checkRequired()) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL insertUser_Coworking_Space(
                        '$this->user_id'
                        ,'$this->coworking_space_id'
                        ,'$this->status'
                            , @ceid)")
    ...
    */
    public function validate( $id )
    {
        try {
            if ( checkInput::checkNewInput($id) ) {
                if ( $this->query->execute("CALL validateUser_Coworking_Space('$id')") ) {
                    $this->query->checkAffected();
                } else {
    ...
    */
    ...
    {
        try {
            if (checkInput::checkNewInputArray(array('$this->user_id','$this->coworking_space_id','$this->status','$this->create_date'))) {
            } else {
