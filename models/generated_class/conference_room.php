...
/******************
Class: Conference_RoomModel
******************/
class Conference_RoomModel extends basicModel implements concreteModelInterface {
    /*** Attributes: ***/
    private $id;
    ...
    static public $Conference_RoomModel;
    static protected $acceptableKeys = array("id", "location_id", "name", "floor", "max_capacity", "status", "create_date", "modify_date", "modify_user", "modify_reason");
    ...
    */
    static public function instance($dbc) {
        if(!isset(self::$Conference_RoomModel) ) {
            self::$Conference_RoomModel = new Conference_RoomModel($dbc);
        } else {
            self::$Conference_RoomModel->connection = $dbc;
        }
        return self::$Conference_RoomModel;
    }
    ...
    public function get( $id ) {
        try {
            if ( checkInput::checkNewInput($id) ) {
    	           if ( $this->query->execute("CALL getConference_Room('$id')") ) {
    	               $this->query->getRow();
    ...
    public function update()
    {
        try {
            if ( $this->checkRequired() ) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL updateConference_Room(
                        '$this->id'
                        ,'$this->location_id'
                        ,'$this->name'
                        ,'$this->floor'
    ...
    */
    public function insert()
    {
        try {
            if ( $this->checkRequired()) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL insertConference_Room(
                        '$this->location_id'
                        ,'$this->name'
                        ,'$this->floor'
                        ,'$this->max_capacity'
    ...
    */
    public function validate( $id )
    {
        try {
            if ( checkInput::checkNewInput($id) ) {
                if ( $this->query->execute("CALL validateConference_Room('$id')") ) {
                    $this->query->checkAffected();
                } else {
    ...
    */
    ...
    {
        try {
            if (checkInput::checkNewInputArray(array('$this->location_id','$this->name','$this->floor','$this->max_capacity','$this->status','$this->create_date'))) {
            } else {
