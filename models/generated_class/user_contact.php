...
/******************
Class: User_ContactModel
******************/
class User_ContactModel extends basicModel implements concreteModelInterface {
    /*** Attributes: ***/
    private $id;
    protected $user_id;
    protected $contact_description_id;
    protected $contact_value;
    protected $status;
    protected $thread;
    ...
    static public $User_ContactModel;
    static protected $acceptableKeys = array("id", "user_id", "contact_description_id", "contact_value", "status", "thread", "create_date", "modify_date", "modify_user", "modify_reason");
    ...
    */
    static public function instance($dbc) {
        if(!isset(self::$User_ContactModel) ) {
            self::$User_ContactModel = new User_ContactModel($dbc);
        } else {
            self::$User_ContactModel->connection = $dbc;
        }
        return self::$User_ContactModel;
    }
        ...
        if(isset($key,$value) AND !is_null($key)){
            switch($key){
                ...
                case 'user_id' :
                    ...
                    break;
                case 'contact_description_id' :
                    if($this->checkValue("int(10) unsigned",10,$value)){
                        $this->contact_description_id=$value;
                    } else {
                        return false;
                    };
                    break;
    ...
    public function get( $id ) {
        try {
            if ( checkInput::checkNewInput($id) ) {
    	           if ( $this->query->execute("CALL getUser_Contact('$id')") ) {
    	               $this->query->getRow();
    ...
    public function update()
    {
        try {
            if ( $this->checkRequired() ) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL updateUser_Contact(
                        '$this->id'
                        ,'$this->user_id'
    ...
    */
    public function insert()
    {
        try {
            if ( $this->checkRequired()) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL insertUser_Contact(
                        '$this->user_id'
                        ,'$this->contact_description_id'
                        ,'$this->contact_value'
                        ,'$this->status'
    ...
    */
    public function validate( $id )
    {
        try {
            if ( checkInput::checkNewInput($id) ) {
                if ( $this->query->execute("CALL validateUser_Contact('$id')") ) {
                    $this->query->checkAffected();
                } else {
    ...
    */
    ...
    {
        try {
            if (checkInput::checkNewInputArray(array('$this->user_id','$this->contact_description_id','$this->contact_value','$this->status','$this->create_date'))) {
            } else {
