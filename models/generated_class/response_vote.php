...
/******************
Class: Response_VoteModel
******************/
class Response_VoteModel extends basicModel implements concreteModelInterface {
    /*** Attributes: ***/
    private $id;
    ...
    static public $Response_VoteModel;
    static protected $acceptableKeys = array("id", "response_id", "user_id", "vote", "create_date", "modify_date", "modify_user", "modify_reason");
    ...
    */
    static public function instance($dbc) {
        if(!isset(self::$Response_VoteModel) ) {
            self::$Response_VoteModel = new Response_VoteModel($dbc);
        } else {
            self::$Response_VoteModel->connection = $dbc;
        }
        return self::$Response_VoteModel;
    }
    ...
    public function get( $id ) {
        try {
            if ( checkInput::checkNewInput($id) ) {
    	           if ( $this->query->execute("CALL getResponse_Vote('$id')") ) {
    	               $this->query->getRow();
    ...
    public function update()
    {
        try {
            if ( $this->checkRequired() ) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL updateResponse_Vote(
                        '$this->id'
                        ,'$this->response_id'
                        ,'$this->user_id'
                        ,'$this->vote'
    ...
    */
    public function insert()
    {
        try {
            if ( $this->checkRequired()) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL insertResponse_Vote(
                        '$this->response_id'
                        ,'$this->user_id'
                        ,'$this->vote'
                            , @ceid)")
    ...
    */
    public function validate( $id )
    {
        try {
            if ( checkInput::checkNewInput($id) ) {
                if ( $this->query->execute("CALL validateResponse_Vote('$id')") ) {
                    $this->query->checkAffected();
                } else {
    ...
    */
    ...
    {
        try {
            if (checkInput::checkNewInputArray(array('$this->response_id','$this->user_id','$this->vote','$this->create_date'))) {
            } else {
