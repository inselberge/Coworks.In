...
Class: CircleModel
******************/
class CircleModel extends basicModel implements concreteModelInterface {
    /*** Attributes: ***/
    private $id;
    protected $user_id;
    protected $title;
    ...
    static public $CircleModel;
    static protected $acceptableKeys = array("id", "user_id", "title", "description", "status", "create_date", "modify_date", "modify_user", "modify_reason");
    ...
    */
    static public function instance($dbc) {
        if(!isset(self::$CircleModel) ) {
            self::$CircleModel = new CircleModel($dbc);
        } else {
            self::$CircleModel->connection = $dbc;
        }
        return self::$CircleModel;
    }
    ...
    public function get( $id ) {
        try {
            if ( checkInput::checkNewInput($id) ) {
    	           if ( $this->query->execute("CALL getCircle('$id')") ) {
    	               $this->query->getRow();
    ...
    public function update()
    {
        try {
            if ( $this->checkRequired() ) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL updateCircle(
                        '$this->id'
                        ,'$this->user_id'
                        ,'$this->title'
                        ,'$this->description'
    ...
    */
    public function insert()
    {
        try {
            if ( $this->checkRequired()) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL insertCircle(
                        '$this->user_id'
                        ,'$this->title'
                        ,'$this->description'
                        ,'$this->status'
    ...
    */
    public function validate( $id )
    {
        try {
            if ( checkInput::checkNewInput($id) ) {
                if ( $this->query->execute("CALL validateCircle('$id')") ) {
                    $this->query->checkAffected();
                } else {
    ...
    */
    ...
    {
        try {
            if (checkInput::checkNewInputArray(array('$this->user_id','$this->title','$this->description','$this->status','$this->create_date'))) {
            } else {
