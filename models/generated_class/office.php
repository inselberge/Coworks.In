...
class OfficeModel extends basicModel implements concreteModelInterface {
    ...
    public function get( $id ) {
        try {
            if ( checkInput::checkNewInput($id) ) {
    	           if ( $this->query->execute("CALL getOffice('$id')") ) {
    	               $this->query->getRow();
    ...
    public function update()
    {
        try {
            if ( $this->checkRequired() ) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL updateOffice(
                        '$this->id'
                        ,'$this->location_id'
                        ,'$this->name'
                        ,'$this->floor'
    ...
    */
    public function insert()
    {
        try {
            if ( $this->checkRequired()) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL insertOffice(
                        '$this->location_id'
                        ,'$this->name'
                        ,'$this->floor'
                        ,'$this->max_capacity'
    ...
    */
    public function validate( $id )
    {
        try {
            if ( checkInput::checkNewInput($id) ) {
                if ( $this->query->execute("CALL validateOffice('$id')") ) {
                    $this->query->checkAffected();
                } else {
    ...
    */
    ...
    {
        try {
            if (checkInput::checkNewInputArray(array('$this->location_id','$this->name','$this->floor','$this->max_capacity','$this->current_capacity','$this->status','$this->create_date'))) {
            } else {
