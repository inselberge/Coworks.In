...
/******************
Class: User_OfficeModel
******************/
class User_OfficeModel extends basicModel implements concreteModelInterface {
    /*** Attributes: ***/
    private $id;
    protected $user_id;
    protected $office_id;
    ...
    protected $modfiy_user;
    protected $modify_reason;
    static public $User_OfficeModel;
    static protected $acceptableKeys = array("id", "user_id", "office_id", "status", "create_date", "modify_date", "modfiy_user", "modify_reason");
    ...
    */
    static public function instance($dbc) {
        if(!isset(self::$User_OfficeModel) ) {
            self::$User_OfficeModel = new User_OfficeModel($dbc);
        } else {
            self::$User_OfficeModel->connection = $dbc;
        }
        return self::$User_OfficeModel;
    }
    ...
    public function get( $id ) {
        try {
            if ( checkInput::checkNewInput($id) ) {
    	           if ( $this->query->execute("CALL getUser_Office('$id')") ) {
    	               $this->query->getRow();
    ...
    public function update()
    {
        try {
            if ( $this->checkRequired() ) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL updateUser_Office(
                        '$this->id'
                        ,'$this->user_id'
                        ,'$this->office_id'
                        ,'$this->status'
    ...
    */
    public function insert()
    {
        try {
            if ( $this->checkRequired()) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL insertUser_Office(
                        '$this->user_id'
                        ,'$this->office_id'
                        ,'$this->status'
                            , @ceid)")
    ...
    */
    public function validate( $id )
    {
        try {
            if ( checkInput::checkNewInput($id) ) {
                if ( $this->query->execute("CALL validateUser_Office('$id')") ) {
                    $this->query->checkAffected();
                } else {
    ...
    */
    ...
    {
        try {
            if (checkInput::checkNewInputArray(array('$this->user_id','$this->office_id','$this->status','$this->create_date'))) {
            } else {
