...
Class: HelpModel
******************/
class HelpModel extends basicModel implements concreteModelInterface {
    /*** Attributes: ***/
    private $id;
    protected $user_id;
    protected $title;
    ...
    static public $HelpModel;
    static protected $acceptableKeys = array("id", "user_id", "title", "description", "estimated_time", "status", "create_date", "modify_date", "modify_user", "modify_reason");
    ...
    */
    static public function instance($dbc) {
        if(!isset(self::$HelpModel) ) {
            self::$HelpModel = new HelpModel($dbc);
        } else {
            self::$HelpModel->connection = $dbc;
        }
        return self::$HelpModel;
    }
    ...
    public function get( $id ) {
        try {
            if ( checkInput::checkNewInput($id) ) {
    	           if ( $this->query->execute("CALL getHelp('$id')") ) {
    	               $this->query->getRow();
    ...
    public function update()
    {
        try {
            if ( $this->checkRequired() ) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL updateHelp(
                        '$this->id'
                        ,'$this->user_id'
                        ,'$this->title'
                        ,'$this->description'
    ...
    */
    public function insert()
    {
        try {
            if ( $this->checkRequired()) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL insertHelp(
                        '$this->user_id'
                        ,'$this->title'
                        ,'$this->description'
                        ,'$this->estimated_time'
    ...
    */
    public function validate( $id )
    {
        try {
            if ( checkInput::checkNewInput($id) ) {
                if ( $this->query->execute("CALL validateHelp('$id')") ) {
                    $this->query->checkAffected();
                } else {
    ...
    */
    ...
    {
        try {
            if (checkInput::checkNewInputArray(array('$this->user_id','$this->title','$this->description','$this->estimated_time','$this->status','$this->create_date'))) {
            } else {
