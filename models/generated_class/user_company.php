...
/******************
Class: User_CompanyModel
******************/
class User_CompanyModel extends basicModel implements concreteModelInterface {
    /*** Attributes: ***/
    private $id;
    ...
    static public $User_CompanyModel;
    static protected $acceptableKeys = array("id", "company_id", "user_id", "status", "create_date", "modify_date", "modify_user", "modify_reason");
    ...
    */
    static public function instance($dbc) {
        if(!isset(self::$User_CompanyModel) ) {
            self::$User_CompanyModel = new User_CompanyModel($dbc);
        } else {
            self::$User_CompanyModel->connection = $dbc;
        }
        return self::$User_CompanyModel;
    }
    ...
    public function get( $id ) {
        try {
            if ( checkInput::checkNewInput($id) ) {
    	           if ( $this->query->execute("CALL getUser_Company('$id')") ) {
    	               $this->query->getRow();
    ...
    public function update()
    {
        try {
            if ( $this->checkRequired() ) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL updateUser_Company(
                        '$this->id'
                        ,'$this->company_id'
                        ,'$this->user_id'
                        ,'$this->status'
    ...
    */
    public function insert()
    {
        try {
            if ( $this->checkRequired()) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL insertUser_Company(
                        '$this->company_id'
                        ,'$this->user_id'
                        ,'$this->status'
                            , @ceid)")
    ...
    */
    public function validate( $id )
    {
        try {
            if ( checkInput::checkNewInput($id) ) {
                if ( $this->query->execute("CALL validateUser_Company('$id')") ) {
                    $this->query->checkAffected();
                } else {
    ...
    */
    ...
    {
        try {
            if (checkInput::checkNewInputArray(array('$this->company_id','$this->user_id','$this->status','$this->create_date'))) {
            } else {
