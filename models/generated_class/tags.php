...
Class: TagsModel
******************/
class TagsModel extends basicModel implements concreteModelInterface {
    /*** Attributes: ***/
    private $id;
    protected $tag;
    protected $status;
    ...
    static public $TagsModel;
    static protected $acceptableKeys = array("id", "tag", "status", "create_date", "modify_date", "modify_user", "modify_reason");
    ...
    */
    static public function instance($dbc) {
        if(!isset(self::$TagsModel) ) {
            self::$TagsModel = new TagsModel($dbc);
        } else {
            self::$TagsModel->connection = $dbc;
        }
        return self::$TagsModel;
    }
    ...
    public function get( $id ) {
        try {
            if ( checkInput::checkNewInput($id) ) {
    	           if ( $this->query->execute("CALL getTags('$id')") ) {
    	               $this->query->getRow();
    ...
    public function update()
    {
        try {
            if ( $this->checkRequired() ) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL updateTags(
                        '$this->id'
                        ,'$this->tag'
                        ,'$this->status'
                        ,'$this->create_date'
    ...
    */
    public function insert()
    {
        try {
            if ( $this->checkRequired()) {
                if ( $this->prewash() ) {
                    if ( $this->query->execute("CALL insertTags(
                        '$this->tag'
                        ,'$this->status'
                            , @ceid)")
                    ) {
    ...
    */
    public function validate( $id )
    {
        try {
            if ( checkInput::checkNewInput($id) ) {
                if ( $this->query->execute("CALL validateTags('$id')") ) {
                    $this->query->checkAffected();
                } else {
    ...
    */
    ...
    {
        try {
            if (checkInput::checkNewInputArray(array('$this->tag','$this->status','$this->create_date'))) {
            } else {
