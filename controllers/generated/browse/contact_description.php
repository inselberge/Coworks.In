 ...
 * @return bool
 */
function browseContact_Description( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewContact_Description " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewContact_Description";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseContact_Description");
                $tb->execute();
            } catch ( Exception $e ) {
