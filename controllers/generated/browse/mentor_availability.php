 ...
 * @return bool
 */
function browseMentor_Availability( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewMentor_Availability " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewMentor_Availability";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseMentor_Availability");
                $tb->execute();
            } catch ( Exception $e ) {
