 ...
 * @return bool
 */
function browseUser_Address( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewUser_Address " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewUser_Address";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseUser_Address");
                $tb->execute();
            } catch ( Exception $e ) {
