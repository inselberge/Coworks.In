 ...
 * @return bool
 */
function browseCoworking_Stripe( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewCoworking_Stripe " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewCoworking_Stripe";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseCoworking_Stripe");
                $tb->execute();
            } catch ( Exception $e ) {
