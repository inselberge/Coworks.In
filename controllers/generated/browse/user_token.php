 ...
 * @return bool
 */
function browseUser_Token( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewUser_Token " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewUser_Token";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseUser_Token");
                $tb->execute();
            } catch ( Exception $e ) {
