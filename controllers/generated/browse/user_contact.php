 ...
 * @return bool
 */
function browseUser_Contact( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewUser_Contact " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewUser_Contact";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseUser_Contact");
                $tb->execute();
            } catch ( Exception $e ) {
