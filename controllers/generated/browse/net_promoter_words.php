 ...
 * @return bool
 */
function browseNet_Promoter_Words( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewNet_Promoter_Words " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewNet_Promoter_Words";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseNet_Promoter_Words");
                $tb->execute();
            } catch ( Exception $e ) {
