 ...
 * @return bool
 */
function browseQuestion( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewQuestion " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewQuestion";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseQuestion");
                $tb->execute();
            } catch ( Exception $e ) {
