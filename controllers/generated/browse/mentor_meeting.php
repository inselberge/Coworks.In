 ...
 * @return bool
 */
function browseMentor_Meeting( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewMentor_Meeting " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewMentor_Meeting";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseMentor_Meeting");
                $tb->execute();
            } catch ( Exception $e ) {
