 ...
 * @return bool
 */
function browseQuestion_Vote( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewQuestion_Vote " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewQuestion_Vote";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseQuestion_Vote");
                $tb->execute();
            } catch ( Exception $e ) {
