 ...
 * @return bool
 */
function browseUser_Stripe( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewUser_Stripe " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewUser_Stripe";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseUser_Stripe");
                $tb->execute();
            } catch ( Exception $e ) {
