 ...
 * @return bool
 */
function browseMonthly_Recurring_Revenue( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewMonthly_Recurring_Revenue " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewMonthly_Recurring_Revenue";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseMonthly_Recurring_Revenue");
                $tb->execute();
            } catch ( Exception $e ) {
