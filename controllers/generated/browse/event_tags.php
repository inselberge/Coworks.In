 ...
 * @return bool
 */
function browseEvent_Tags( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewEvent_Tags " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewEvent_Tags";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseEvent_Tags");
                $tb->execute();
            } catch ( Exception $e ) {
