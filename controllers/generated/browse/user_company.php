 ...
 * @return bool
 */
function browseUser_Company( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewUser_Company " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewUser_Company";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseUser_Company");
                $tb->execute();
            } catch ( Exception $e ) {
