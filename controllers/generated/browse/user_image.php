 ...
 * @return bool
 */
function browseUser_Image( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewUser_Image " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewUser_Image";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseUser_Image");
                $tb->execute();
            } catch ( Exception $e ) {
