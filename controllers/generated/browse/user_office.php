 ...
 * @return bool
 */
function browseUser_Office( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewUser_Office " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewUser_Office";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseUser_Office");
                $tb->execute();
            } catch ( Exception $e ) {
