 ...
 * @return bool
 */
function browseUser_Login( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewUser_Login " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewUser_Login";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseUser_Login");
                $tb->execute();
            } catch ( Exception $e ) {
