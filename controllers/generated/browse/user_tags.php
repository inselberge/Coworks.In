 ...
 * @return bool
 */
function browseUser_Tags( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewUser_Tags " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewUser_Tags";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseUser_Tags");
                $tb->execute();
            } catch ( Exception $e ) {
