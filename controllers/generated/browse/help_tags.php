 ...
 * @return bool
 */
function browseHelp_Tags( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewHelp_Tags " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewHelp_Tags";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseHelp_Tags");
                $tb->execute();
            } catch ( Exception $e ) {
