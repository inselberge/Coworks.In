 ...
 * @return bool
 */
function browseCompany_Tags( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewCompany_Tags " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewCompany_Tags";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseCompany_Tags");
                $tb->execute();
            } catch ( Exception $e ) {
