 ...
 * @return bool
 */
function browsePaying_Customers( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewPaying_Customers " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewPaying_Customers";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browsePaying_Customers");
                $tb->execute();
            } catch ( Exception $e ) {
