 ...
 * @return bool
 */
function browseQuestion_Tags( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewQuestion_Tags " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewQuestion_Tags";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseQuestion_Tags");
                $tb->execute();
            } catch ( Exception $e ) {
