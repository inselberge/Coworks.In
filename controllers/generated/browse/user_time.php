 ...
 * @return bool
 */
function browseUser_Time( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewUser_Time " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewUser_Time";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseUser_Time");
                $tb->execute();
            } catch ( Exception $e ) {
