 ...
 * @return bool
 */
function browseLink_Tags( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewLink_Tags " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewLink_Tags";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseLink_Tags");
                $tb->execute();
            } catch ( Exception $e ) {
