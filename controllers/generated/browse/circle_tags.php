 ...
 * @return bool
 */
function browseCircle_Tags( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewCircle_Tags " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewCircle_Tags";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseCircle_Tags");
                $tb->execute();
            } catch ( Exception $e ) {
