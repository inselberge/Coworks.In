 ...
 * @return bool
 */
function browseUser_Type( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewUser_Type " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewUser_Type";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseUser_Type");
                $tb->execute();
            } catch ( Exception $e ) {
