 ...
 * @return bool
 */
function browseUser_Invite( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewUser_Invite " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewUser_Invite";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseUser_Invite");
                $tb->execute();
            } catch ( Exception $e ) {
