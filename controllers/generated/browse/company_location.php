 ...
 * @return bool
 */
function browseCompany_Location( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewCompany_Location " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewCompany_Location";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseCompany_Location");
                $tb->execute();
            } catch ( Exception $e ) {
