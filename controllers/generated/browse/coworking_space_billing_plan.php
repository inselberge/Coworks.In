 ...
 * @return bool
 */
function browseCoworking_Space_Billing_Plan( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewCoworking_Space_Billing_Plan " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewCoworking_Space_Billing_Plan";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseCoworking_Space_Billing_Plan");
                $tb->execute();
            } catch ( Exception $e ) {
