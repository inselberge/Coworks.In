 ...
 * @return bool
 */
function browseUser_Coworking_Space( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewUser_Coworking_Space " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewUser_Coworking_Space";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseUser_Coworking_Space");
                $tb->execute();
            } catch ( Exception $e ) {
