 ...
 * @return bool
 */
function browseLocation( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewLocation " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewLocation";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseLocation");
                $tb->execute();
            } catch ( Exception $e ) {
