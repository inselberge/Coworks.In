 ...
 * @return bool
 */
function browseLocation_User_Status( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewLocation_User_Status " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewLocation_User_Status";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseLocation_User_Status");
                $tb->execute();
            } catch ( Exception $e ) {
