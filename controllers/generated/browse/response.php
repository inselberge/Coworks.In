 ...
 * @return bool
 */
function browseResponse( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewResponse " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewResponse";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseResponse");
                $tb->execute();
            } catch ( Exception $e ) {
