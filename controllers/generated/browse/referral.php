 ...
 * @return bool
 */
function browseReferral( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewReferral " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewReferral";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseReferral");
                $tb->execute();
            } catch ( Exception $e ) {
