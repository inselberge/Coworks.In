 ...
 * @return bool
 */
function browseEvent_Attendance( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewEvent_Attendance " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewEvent_Attendance";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseEvent_Attendance");
                $tb->execute();
            } catch ( Exception $e ) {
