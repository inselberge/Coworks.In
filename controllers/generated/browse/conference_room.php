 ...
 * @return bool
 */
function browseConference_Room( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewConference_Room " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewConference_Room";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseConference_Room");
                $tb->execute();
            } catch ( Exception $e ) {
