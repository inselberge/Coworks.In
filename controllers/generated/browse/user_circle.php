 ...
 * @return bool
 */
function browseUser_Circle( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewUser_Circle " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewUser_Circle";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseUser_Circle");
                $tb->execute();
            } catch ( Exception $e ) {
