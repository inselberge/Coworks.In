 ...
 * @return bool
 */
function browseUser_Bio( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewUser_Bio " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewUser_Bio";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseUser_Bio");
                $tb->execute();
            } catch ( Exception $e ) {
