 ...
 * @return bool
 */
function browseUser_Application( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewUser_Application " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewUser_Application";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseUser_Application");
                $tb->execute();
            } catch ( Exception $e ) {
