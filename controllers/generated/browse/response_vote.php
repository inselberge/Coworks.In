 ...
 * @return bool
 */
function browseResponse_Vote( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewResponse_Vote " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewResponse_Vote";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseResponse_Vote");
                $tb->execute();
            } catch ( Exception $e ) {
