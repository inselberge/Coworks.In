 ...
 * @return bool
 */
function browseUser_Cave( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewUser_Cave " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewUser_Cave";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseUser_Cave");
                $tb->execute();
            } catch ( Exception $e ) {
