 ...
 * @return bool
 */
function browseUser_Net_Promoter_Score( $dbc, $argsMySQL = null )
{
    if ( isset($dbc) ) {
        if ( isset($argsMySQL) AND !is_null($argsMySQL) ) {
            $q = "SELECT * FROM viewUser_Net_Promoter_Score " . $argsMySQL;
        } else {
            $q = "SELECT * FROM viewUser_Net_Promoter_Score";
        }
        ...
        if ( $rows > 0 ) {
            ...
            try {
                $tb = new tableBuilder($dbc, $b_result, "browseUser_Net_Promoter_Score");
                $tb->execute();
            } catch ( Exception $e ) {
